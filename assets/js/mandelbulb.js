...
export function initMandelbulb(containerId = 'mandelbulb-bg') {
        ...
        fragmentShader: `
            ...
            float mandelbulb(vec3 c) {
                ...
                for (int i = 0; i < MAX_ITER; i++) {
                    ...
                    float ndr = 8.0 * pow(r, 7.0) * dr + 1.0;

                    float a = atan(sqrt(zr * zr + zi * zi), zj);
                    float b = 8.0 * a;
                    float r_n = pow(r, 8.0);

                    z = r_n * vec3(sin(b) * cos(8.0 * atan(zi, zr)), sin(b) * sin(8.0 * atan(zi, zr)), cos(b));
                    z += c;
                    dr = ndr;
                }
    ...
    function onResize() {
        ...
        renderer.setSize(container.clientWidth, container.clientHeight);
        mandelbulbMaterial.uniforms.resolution.value.set(container.clientWidth, container.clientHeight);
    }
